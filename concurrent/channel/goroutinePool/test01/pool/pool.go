...
func New(capacity int, opt ...Option) *Pool {
	if capacity <= 0 {
...
func (p *Pool) run() {
	...
	// 未预先分配，这个时候塞进来1000任务，就会不断newWorker，直到capacity
	// 如果只来了2个任务，就new 2个 worker
	fmt.Println("等任务来,根据任务创建worker")
	if !p.preAlloc {
	loop:
		for t := range p.tasks {
			// 外面的for循环读出了task，这里需要塞回去
			p.returnTask(t)
			select {
			case <-p.quit:
				return
