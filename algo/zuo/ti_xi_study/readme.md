...
思路：上一个题是统计右边有多少个数比当前数大，这个题是统计右边有多少个数比当前数小；关键是分组里面的下标从右到左遍历。
...
2的后面没有
所以总共有5个
思路：通过归并排序的思路，对于左组每个数，遍历右组并利用窗口滑动的方式，找到满足条件的数，然后将左组的数和右组的数合并，最后返回总个数。
...
思路：第一步是计算出数组的前缀和数组，因为`Sum(i,j) = preSum[j] - preSum[i]`，所以现在假定一个数字num下标对应j，需要寻找`preSum[0~j]`有多少落在`[preSum[j]-upper,preSum[j]-lower]`范围上。假如j=17，`preSum[17]=100`，lower=10，upper=40，那么就是找`preSum[0~17]`位置上的前缀和有多少个落在`[60,90]`范围上，比如`preSum[5]=70`那么`preSum[6~17]=100-70=30`就是在`[10,40]`范围上。所以我们就是求每个位置的满足条件的前缀和，然后累计就是答案了。

荷兰国旗问题的实现
思路：给定一个数组arr，一个数num，将数组中小于num的数放在数组的左边，等于num的数放在数组的中间，大于num的数放在数组的右边。这个问题是快速排序的基础，快速排序就是将数组中的数按照某个数分成左右两部分，然后再对左右两部分进行同样的操作，直到整个数组有序。这个问题的实现就是快速排序的基础，通过一个指针遍历数组，将数组分成三部分，小于num的数放在左边，等于num的数放在中间，大于num的数放在右边。
