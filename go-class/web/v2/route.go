...
// 定义为私有的 addRoute：用户只能通过 Get 或者 Post 方法来注册，那么可以确保 method 参数永远都是对的
func (r *router) addRoute(method string, path string, handler HandleFunc) {
	...
	// 此时root指向path最后一段，也就是叶子节点才会增加业务处理逻辑，如果原来 handler!=nil 说明被注册了，会报路由冲突
	if root.handler != nil {
...
func (r *router) findRoute(method string, path string) (*node, bool) {
	...
	for _, s := range segs {
		// 不断更新root，此时找到的root作为下一个node的父节点
